...
/*
void TimeLine::re_build_chessplate(int piece_index,
				   ...
				   std::vector<int> indexs)
{
  ...
  if(chemin.size() == 1)
  {
    cout << " x: "<<chemin[0].c.x<<"  y: "<<chemin[0].c.x<<endl;
    for(int i = 0; i < indexs.size(); i++)
      cout << "  inx: "<<indexs[i]<<endl;
  }
  Coord c1,c2;
  Piece* piece;
  Piece* k_piece;
  ...
  while(i < instants.size())
  {
    if(i == indexs[h])
    {
      piece = chessplate->at(piece_index);

      k_piece = chessplate->piece_at_coord(chemin[h].c.x,
					   chemin[h].c.y);
      if(k_piece != NULL)
      {
	if(k_piece->get_Color() != piece->get_Color())
	{
	  k_piece->set_Alive(false);
	  add_instant_at_mod_color(piece, Action::eat,Info(),i);
	}
	else
	{
	  score_kill();
      ...
      {
	add_instant_at_mod_color(piece, Action::move,Info(),i);
      }
      
      piece->add_movements(i, chemin[h].c);
      h++;
    }
    else
    {
	...
	if(tm[j] == i)
	{
      ...
      piece = chessplate->piece_at_coord(c1.x,c1.y);
	...
	if(piece->Test_movements(&c2, instants[i].a == eat, piece->get_TM_size()-1))
	{
	  ...
	  {
	  case tours:
	    if(instants[i].a != change && !chessplate->check_Rok_path(c1,c2 ))
	    {
	      score_kill();
	      return;
	    }  
	    break;
	  case fous:
	    if(chessplate->check_Bishop_path(c1,c2))
	    {
	      score_kill();
	      return;
	    }
	    break;
	  case dame:
	    if(!chessplate->check_Rok_path(c1,c2)
	       && !chessplate->check_Bishop_path(c1,c2))
	    {
	      score_kill();
	      return;
	    }
	...
	return;
      ...
      if(instants[i].a == eat)
      {
	k_piece = chessplate->piece_at_coord(c2.x,c2.y);
	
	if(k_piece == NULL && piece->get_Type() == pions)
	{
	  if(piece->get_Color())
	    k_piece = chessplate->piece_at_coord(c2.x,c2.y-1);
	  else
	    k_piece = chessplate->piece_at_coord(c2.x,c2.y+1);
	}
